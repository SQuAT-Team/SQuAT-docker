...
/**
 * Provides the responses from the LQN solver.
 */
public class LQNSResult {
	...
	// copied from LQNResult, but without parent constructor (Opt4JStarter
	// dependency)
	public LQNSResult(PCMInstance pcm, LqnModelType model, Criterion criterion) throws AnalysisFailedException {
		try {
			if (criterion instanceof UsageScenarioBasedCriterion) {
				this.meanResponseTime = retrieveResponseTimeForUsageScenario(pcm, model,
						(UsageScenarioBasedCriterion) criterion);
				this.throughput = retrieveThroughputForUsageScenario(pcm, model,
						(UsageScenarioBasedCriterion) criterion);
			} else if (criterion instanceof EntryLevelSystemCallCriterion) {
				this.meanResponseTime = retrieveResponseTimeForEntryLevelSystemCall(pcm, model,
						(EntryLevelSystemCallCriterion) criterion);
				this.throughput = retrieveThroughputForEntryLevelSystemCall(pcm, model,
						(EntryLevelSystemCallCriterion) criterion);
			} else {
			...
			throw new AnalysisFailedException("Failed to parse string value.", ex);
		}
	}

	private double retrieveMaxUtilization(PCMInstance pcm, LqnModelType model, ResultDecoratorRepository results2,
		...
		for (UtilisationResult utilResult : results2.getUtilisationResults_ResultDecoratorRepository()) {
			if (utilResult instanceof ProcessingResourceSpecificationResult) {
				...
				// check resource type
				if (procResult.getProcessingResourceSpecification_ProcessingResourceSpecificationResult()
						.getActiveResourceType_ActiveResourceSpecification().getEntityName()
						.contains(resourceTypeDescription)) {
	...
	private ResultDecoratorRepository retrieveResults(PCMInstance pcm, LqnModelType model) throws ParseException {
		...
		List<ProcessorType> modifiableProcessorList = retrieveResourceEnvironmentResults(pcm, model, repo);
	...
	private void retrieveServiceResults(PCMInstance pcm, ResultDecoratorRepository repo,
		...
		for (Repository repository : repositories) {
			...
			for (RepositoryComponent repositoryComponent : repoComponentList) {
				if (repositoryComponent instanceof BasicComponent) {
					...
					// pass the processor list that has already been used, then
					// it does not contain the resources anymore.
	...
	private List<ProcessorType> retrieveResourceEnvironmentResults(PCMInstance pcm, LqnModelType model,
			ResultDecoratorRepository repo) throws ParseException {
		...
		// create a new list from that we can remove things without modifying
		// the model
		...
		for (ResourceContainer container : containers) {
			for (ProcessingResourceSpecification pcmResource : container
					.getActiveResourceSpecifications_ResourceContainer()) {
				String processorID = Pcm2LqnHelper.getIdForProcResource(container,
						pcmResource.getActiveResourceType_ActiveResourceSpecification()) + "_Processor";

				ProcessingResourceSpecificationResult result = ResourceenvironmentdecoratorFactory.eINSTANCE
						.createProcessingResourceSpecificationResult();

				retrieveUtilResultFromLQN(modifiableProcessorList, processorID, result);
				// if the resource has not been found, it has not been used and
				// its utilisation is 0 (the default value).
		...
		for (LinkingResource linkingResource : links) {
			String processorID = Pcm2LqnHelper
					.getIdForCommResource(linkingResource,
			...
			LinkingResourceResults result = ResourceenvironmentdecoratorFactory.eINSTANCE
					.createLinkingResourceResults();

			retrieveUtilResultFromLQN(modifiableProcessorList, processorID, result);
			// if the resource has not been found, it has not been used and its
			// utilisation is 0 (the default value).
		...
		for (AllocationContext allocContext : allAssemblyContexts) {
			...
			if (innerComponent instanceof BasicComponent) {
				...
				for (PassiveResource passiveResource : passiveResourceOfComponentList) {
					...
					PassiveResourceResult result = ResourceenvironmentdecoratorFactory.eINSTANCE
							.createPassiveResourceResult();

					retrieveUtilResultFromLQN(modifiableProcessorList, passiveResourceId, result);
					// if the resource has not been found, it has not been used
					// and its utilisation is 0 (the default value).
	...
	/**
	 * Sets utilization, average queue length and average waiting time, if
	 * available.
	 * 
	 ...
	 * @throws ParseException
	 */
	private void retrieveUtilResultFromLQN(List<ProcessorType> modifiableProcessorList, String processorID,
		...
		while (lqnProcessorIterator.hasNext()) {
			...
			if (lqnProc.getName().contains(processorID)) {
				...
				if (lqnResultProc.size() > 0) {
					...
					// determine waiting times and service times by checking all
					// result entries (contained in the first task)
					...
					if (resultTask.size() > 0) {
						...
						for (EntryType entryType : procTaskEntries) {
							...
							if (entryResults.size() > 0) {
								...
								EList<ActivityPhasesType> entryPhaseActivities = entryType.getEntryPhaseActivities()
										.getActivity();
								if (entryPhaseActivities.size() > 0) {
									List<OutputResultType> activityResults = entryPhaseActivities.get(0)
											.getResultActivity();
									if (activityResults.size() > 0) {
										...
										// only look at service times greater
										// than one, because the others do not
										// seem to have to wait.
										// weight the current waiting time and
										// service time by the current
										// throughgput to get the overall times.
						...
						if (totalThroughput > 0 && serviceTime > 0) {
							// weight waiting time and service time by
							// throughput.
							waitingTime = waitingTime / totalThroughput;
							serviceTime = serviceTime / totalThroughput;

							// queue length is queue waiting time / queue
							// service time in average
					...
					logger.debug("Resource " + processorID + " has utilisation " + processorResult.getUtilization()
							+ " and waiting time " + result.getAverageQueueLength());
				}

				// we can remove this element now and do not have to iterate
				// over this one again in the next loop iterations.
				lqnProcessorIterator.remove();
			}
		}
	}

	private List<ServiceResult> getServiceResults(BasicComponent basicComponent,
			List<ProcessorType> modifiableProcessorResultList) throws ParseException {
		...
		// Create only one service result per SEFF: weight each response time by
		// throughput and then divide by overall throughput.
		for (ServiceEffectSpecification seff : seffList) {
			...
			String processorIDRegex = basicComponent.getEntityName() + "_"
					+ ((OperationSignature) seff.getDescribedService__SEFF()).getInterface__OperationSignature()
							.getEntityName()
					+ "_" + seff.getDescribedService__SEFF().getEntityName(); // +
																				// "_*_Processor";
			...
			while (itProcessors.hasNext()) {
				...
				if (proc.getName().contains(processorIDRegex) && proc.getName().contains("_Processor")) {
					...
					if (taskList.size() > 0) {
						...
						List<EntryType> procTaskEntries = taskList.get(0).getEntry();
						for (EntryType entryType : procTaskEntries) {
							List<OutputResultType> entryResults = entryType.getResultEntry();
							if (entryResults.size() > 0) {
	 ...
	 */
	private double retrieveResponseTimeForUsageScenario(PCMInstance pcm, LqnModelType model,
			UsageScenarioBasedCriterion criterion) throws ParseException, AnalysisFailedException {
		...
		logger.warn("No task or empty task for processor " + processor.getName()
	...
	private double retrieveResponseTimeForEntryLevelSystemCall(PCMInstance pcm, LqnModelType model,
			EntryLevelSystemCallCriterion criterion) throws AnalysisFailedException, ParseException {
		...
		if (usageScenarioProcessor != null) {
			...
			if (usageScenarioProcessor.getTask() != null && usageScenarioProcessor.getTask().size() > 0) {
				...
				OutputResultType entryLevelCallActivityResult = getResultActivityForEntryLevelSystemCall(criterion,
						usageScenarioProcessor);
		...
		logger.warn("No task or empty task for processor or empty activity results" + usageScenarioProcessor.getName()
	...
	private OutputResultType getResultActivityForEntryLevelSystemCall(EntryLevelSystemCallCriterion criterion,
		...
		TreeIterator<Object> allProcessorContentIterator = EcoreUtil
				.getAllContents(task.getTaskActivities().getActivity(), true);
		OutputResultType activity = findActivityForEntryLevelCallById(entryLevelCallId, allProcessorContentIterator);

		if (activity == null) {

			// the activity might be contained in another processor, e.g. a
			// usage scenario loop processor.
			TreeIterator<Object> allContentIterator = EcoreUtil.getAllContents(usageScenarioProcessor.eContainer(),
					true);
	...
	private OutputResultType findActivityForEntryLevelCallById(String entryLevelCallId,
		...
		while (allProcessorContentIterator.hasNext()) {
			...
			if (object instanceof ActivityDefType && ((ActivityDefType) object).getName().equals(entryLevelCallId)) {
				...
				if (((ActivityDefType) object).getResultActivity() != null
						&& ((ActivityDefType) object).getResultActivity().size() > 0) {
	 ...
	 * @throws ParseException
	 */
	private double retrieveThroughputForUsageScenario(PCMInstance pcm, LqnModelType model,
			UsageScenarioBasedCriterion criterion) throws AnalysisFailedException, ParseException {
		...
		logger.warn("No task or empty task for processor or empty activity results" + processor.getName()
				+ ". Cannot determine throughput, using NaN. Check your models or the LQNResult code.");
		return Double.NaN;

	}

	private double retrieveThroughputForEntryLevelSystemCall(PCMInstance pcm, LqnModelType model,
			EntryLevelSystemCallCriterion criterion) throws AnalysisFailedException, ParseException {
		ProcessorType usageScenarioProcessor = getUsageScenarioProcessor(pcm, model, criterion);
	...
	private ProcessorType getUsageScenarioProcessor(PCMInstance pcm, LqnModelType model,
			UsageScenarioBasedCriterion criterion) throws AnalysisFailedException {
	...
	private ProcessorType getUsageScenarioProcessor(PCMInstance pcm, LqnModelType model,
			EntryLevelSystemCallCriterion criterion) throws AnalysisFailedException {
		UsageScenario currentUsageScenaro = null;
		List<UsageScenario> scenarios = pcm.getUsageModel().getUsageScenario_UsageModel();
	...
	/**
	 * Get usage scenario processor for a given {@link UsageScenario}. I am not
	 * sure here, but maybe we have to be careful and the {@link UsageScenario}
	 * must be the one from the actual PCM model that has been analyzed, not the
	 * one referenced from the Criterion. These may differ if they have been
	 * loaded separately several Java objects may represent the same model
	 * objects.
	 * 
	 * @param pcm
	 * @param model
	 ...
	 */
	private ProcessorType getUsageScenarioProcessor(PCMInstance pcm, LqnModelType model,
			UsageScenario currentUsageScenaro) throws AnalysisFailedException {

		if (currentUsageScenaro == null) {
			throw new AnalysisFailedException(
					"Could not analyse LQN results, because the usage scenario from references QML Criterion is not contained in this PCM model's usage model.");
		}
		...
		String processorNameRegex = scenarioName + "_\\d+_Processor"; // see
																		// class
																		// LqnBuilder#getProcessorTypeFromModel(String)

		// Obtain processor, representing the usage scenario's overall resource
		// demand
		ProcessorType processor = null;
		...
		if (processor == null) {
			logger.warn("Could not find LQN processor matching name " + processorNameRegex
					+ ". Cannot determine response time, using NaN. Check your models or the LQNResult code.");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.uka.ipd.sdq.dsexplore.analysis.lqn.ILQNResult#
	 ...
	 * 
	 * @see
	 * de.uka.ipd.sdq.dsexplore.analysis.lqn.ILQNResult#getCoefficientOfVariance
	 * ()
	 */
	 ...
	 * 
	 * @see
	 * de.uka.ipd.sdq.dsexplore.analysis.lqn.ILQNResult#getStandardDeviation()
	 */
