...
public class ArchitecturalTransformationsFactory {
	...
	public CompletableFuture<List<RestArchitecture>> getArchitecturalTransformationsUntilLevel(int n) {
		return createArchitecturalTransformationsForLevel(n).thenApplyAsync(v -> {
			...
			for (int i = 1; i <= n; i++) {
				this.resultPerLevel.get(i).stream().map(RestScenarioResult::getArchitecture).forEach(ret::add);
			}
			return ret;
		});
	}

	public List<RestArchitecture> returnForLevel(int n) {
		List<RestArchitecture> ret = new ArrayList<>();
		if (this.resultPerLevel.get(n) == null)
			return ret;
		for (int i = 1; i <= n; i++) {
			this.resultPerLevel.get(i).stream().map(RestScenarioResult::getArchitecture).forEach(ret::add);
		}
	...
	private CompletableFuture<Void> performPerfBot(RestArchitecture arch, List<RestScenarioResult> results) {
		...
		return CompletableFuture.allOf(ress.toArray(new CompletableFuture<?>[ress.size()]));

		// performanceBots.stream().map(b -> b.searchForAlternatives(arch)).map(fut ->
		// fut.thenAccept(results::addAll));
	}
