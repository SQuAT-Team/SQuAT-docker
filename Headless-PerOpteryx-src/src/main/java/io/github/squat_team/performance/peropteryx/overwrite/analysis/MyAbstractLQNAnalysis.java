...
public abstract class MyAbstractLQNAnalysis extends MyAbstractAnalysis implements IAnalysis {
	...
	public MyAbstractLQNAnalysis(){
		super(new MyLQNQualityAttributeDeclaration());
	}
	...
	public IAnalysisResult retrieveResultsFor(PCMPhenotype pheno, Criterion criterion) throws AnalysisFailedException{
		...
		// NOTE: Added for Headless PerOpteryx to persist PCMs, this method is always called twice
		if(pheno.getGenotypeID().equals(lastID)){
			//pcm.saveToFiles("candidate" + new Random().nextDouble());
			PCMFileExporter.getInstance().savePCM(pheno, pcm);
		}
		lastID = pheno.getGenotypeID();
	...
	ILQNResult retrieveLQNSolverResults(PCMPhenotype pheno, PCMInstance pcm, Criterion criterion) throws AnalysisFailedException {
		...
		if (model == null){
			...
			model =  MyLqnXmlHandler.loadModelFromXMI(resultFileName);
		}
